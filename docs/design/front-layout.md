...
Names and handles stay on one line: each value shrinks from its maximum size to its minimum size until it fits its unchanged width, then uses a grapheme-safe ellipsis if it still does not fit. The display name shrinks to 36px; the handle shrinks to 32px. Both footer lines use fixed 16px text; roles remain 32px and ellipsize.
