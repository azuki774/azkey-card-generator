...
`config/roles.csv.b64` は、カードのロール表示用ファイルです。起動時に同梱ファイルを
デフォルトで読み込むため、コンテナでも追加設定は不要です。別ファイルを使う場合だけ
`CARD_ROLES_FILE` でパスを指定してください。読み込み・形式のエラーは起動時に検出します。
ファイルの変更を反映するには再起動が必要です。
